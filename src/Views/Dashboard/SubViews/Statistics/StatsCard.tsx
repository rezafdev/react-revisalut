...
const ChartRed = () => (
    <svg width="157" height="107" viewBox="0 0 157 107" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            d="M90.7738 43.2928C124.19 15.1026 149.688 45.3022 156 76.9515V107H1V1C7.66289 15.1601 20.9887 102.164 90.7738 43.2928Z"
            fill="url(#paint0_linear_394_10109)"/>
        <path d="M156 76.9515C149.688 45.3022 124.19 15.1026 90.7738 43.2928C20.9887 102.164 7.6629 15.1601 1 1"
              stroke="#753F3F" strokeWidth="2" strokeLinecap="round"/>
        <defs>
            <linearGradient id="paint0_linear_394_10109" x1="78.5" y1="1" x2="78.5" y2="107"
                            gradientUnits="userSpaceOnUse">
                <stop stopColor="#D02626" stopOpacity="0.08"/>
                <stop offset="1" stopColor="#D02626" stopOpacity="0"/>
            </linearGradient>
        </defs>
    </svg>
)
...
const StatsCard = (props: {
    ...
    return (
        <div
            className={`${!!props.colSpan ? `col-span-${props.colSpan}` : 'col-span-5'} shadow-sm w-full h-full bg-white rounded rounded-xl flex flex-row justify-between items-center p-6`}>
            <div className='flex flex-col items-start gap-1'>
                ...
                <div className='flex flex-row justify-start items-center gap-x-1'>
                    ...
                    <span
                        className={`font-medium text-sm ${_.toNumber(props.change) < 0 ? 'text-[#753F3F]' : 'text-[#01505F]'}`}>{props.change}{props.changeSymbol ?? ''}</span>
                    <span className='text-[#8B909A] font-medium text-sm ml-1'>{t('VsLast7Days')}</span>
                </div>
            </div>

            <div>
                {props.change > 0 ? <ChartGreen/> : <ChartRed/>}
            </div>
        </div>
    )
}
